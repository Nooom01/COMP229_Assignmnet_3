...
export default function ManageQualifications() {
  ...
  const fetchQualifications = async () => {
    try {
      ...
      toast.error('Error fetching qualifications');
  ...
  const onSubmit = async (data) => {
    try {
      if (editing) {
        await axios.put(`http://localhost:5000/api/qualifications/${editing}`, data);
        toast.success('Qualification updated successfully');
        setEditing(null);
      } else {
        await axios.post('http://localhost:5000/api/qualifications', data);
  ...
  const handleEdit = (qualification) => {
    setEditing(qualification._id);
    setShowForm(true);
    Object.keys(qualification).forEach(key => {
      setValue(key, qualification[key]);
    });
  };
  ...
  if (loading) {
    return <div style={styles.loading}>Loading qualifications...</div>;
  }

  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>Manage Qualifications</h1>
      ...
      {showForm && (
        <div style={styles.formContainer}>
          <h2>{editing ? 'Edit Qualification' : 'Add New Qualification'}</h2>
      ...
      {qualifications.length === 0 ? (
        <p style={styles.noData}>No qualifications found.</p>
      ) : (
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {qualifications.map(qual => (
                <tr key={qual._id}>
                  <td style={styles.td}>{qual.title}</td>
                  <td style={styles.td}>{qual.firstname} {qual.lastname}</td>
                  <td style={styles.td}>{qual.email}</td>
                  <td style={styles.td}>
                    {new Date(qual.completion).toLocaleDateString()}
                  </td>
                  <td style={styles.td}>
                    <button onClick={() => handleEdit(qual)} style={styles.editButton}>
                      Edit
                    </button>
                    <button onClick={() => handleDelete(qual._id)} style={styles.deleteButton}>
                      Delete
                    </button>
                  </td>
                </tr>
